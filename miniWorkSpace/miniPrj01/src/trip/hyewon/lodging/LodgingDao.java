...
public class LodgingDao {
	...
	public List<LodgingVo> showPopularLodgingList(Connection conn) throws Exception {
		...
		String sql = "SELECT LI.NO, NAME, ADDRESS, COUNT(LW.LODGING_NO) FROM LODGING_WISH LW JOIN LODGING_INFORMATION LI ON LW.LODGING_NO = LI.NO WHERE LW.CANCEL_YN = 'N' GROUP BY NAME, ADDRESS, LI.NO ORDER BY COUNT(LW.LODGING_NO) DESC OFFSET 0 ROWS FETCH NEXT 10 ROWS ONLY";
